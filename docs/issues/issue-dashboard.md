...
|---|---|---|---|---|---|
| I-006 | Large-File Documentation Truncation Risk | In Progress | `docs/roadmap.md` Phase 1; WBS `3.9` | Mitigate the current `maxFileBytes` plus full-file-response contract so a docs-safe update cannot delete unseen tail content from large markdown files, then add repeatable regression coverage before resuming WBS `3.9` close-out | Move out after the execution contract is hardened and the regression is covered by durable testing |
...
- Current State: Fresh evidence from issue `#41` / comment `#193` -> task `trq-097cad6b8f77` -> session `ags-2736c300be71` -> PR `#42` -> commit `83acd236a04c1d59dbf109c964e389308b53a053` shows exactly that failure mode on `README.md`. The repo-owned execution contract currently truncates context files at `maxFileBytes: 8000` while still asking the provider to return complete file contents, and the current `README.md` is `11606` bytes. The resulting diff added a small weather note but also deleted the unseen tail of the file.
- Next Action: Choose and implement the narrowest mitigation that prevents full-file destructive rewrite when the model has only partial file context, then add repeatable regression coverage.
