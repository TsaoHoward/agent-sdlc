...
|---|---|---|---|---|---|
| I-001 | Phase 1 Minimum Closed Loop Implementation Packaging | In Progress | `docs/roadmap.md` Phase 1; WBS `3`, `3.1`-`3.11` | Keep the fresh `TC-008` acceptance evidence, the P1 deliverable, and the user-facing guides current; preserve the CI-to-host traceability callback path; and decide whether bounded-code follow-up should be prompt hardening, rubric tightening, or a narrower operator runbook rule | Archive after the first slice is underway and the remaining work is split or absorbed into active execution tracking |
| I-005 | Issue-Comment Failure And No-Op UX Hardening | Deferred | `docs/roadmap.md` Phase 1; WBS `3.1`, `3.8`, `3.9` | Keep the newly landed bounded issue-thread feedback path minimal in P1, then design richer user-facing status, retry guidance, duplicate-comment control, operational-state storage evolution, and unified UX-discovery coverage as a follow-up instead of hiding it in chat | Move out after the follow-up is either promoted into active implementation or intentionally deferred beyond the current P1 packaging window |
...
- Next Action: Keep the new P1 manual acceptance pack (`docs/phase1-deliverable.md` plus `TC-008`) as the default near-term confirmation path now that it has one fresh accepted rerun, continue hardening the provider-enabled DeepSeek path from the validated session/proposal smoke, keep `docs/testing/`, `docs/user-capability-matrix.md`, and the new concise Traditional Chinese guide aligned with actual workflow behavior, preserve the CI-to-host traceability callback path during future proposal/CI changes, and use the docs-vs-code comparison to decide whether bounded-code follow-up should be prompt hardening, rubric tightening, or both.
...
- Current State: The minimal P1 fix is now in place: fail-closed intake and failed live-continuation states can write bounded `agent-admin` comments back into the issue thread, and a completed zero-edit agent result now stops before PR creation instead of silently creating a traceability-only PR. The richer design space is intentionally left open so P1 does not expand into a full conversational control plane by accident.
- Next Action: Keep the landed feedback path narrow during P1, then choose whether the next follow-up should add structured status taxonomy, duplicate-comment suppression, explicit retry links, richer no-op explanation, operational-state storage evolution beyond file-backed repo state, or a more systematic UX discovery/acceptance mechanism.
