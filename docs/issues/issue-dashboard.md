...
| I-002 | Configuration Template Policy Compliance Gaps | In Progress | ADR-0008; `docs/policies/configuration-management.md`; WBS `3.1`, `3.3`, `3.5`, `3.9` | Fold runner, worker-runtime, and CI callback settings into template-backed configuration surfaces where they are operator-controlled | Close after all current operator-controlled startup settings either follow the template/local-config pattern or have an explicit documented exception |
...
- Current State: `config/agent-execution.template.yaml` plus ignored `config/agent-execution.yaml` is compliant. Machine-readable policy under `config/policy/` remains committed source-of-truth policy input and does not need a template/local split unless policy ownership changes. The local Gitea/dev bootstrap now follows the ADR-0008 pattern with tracked `config/dev/gitea-bootstrap.template.json`, ignored generated `config/dev/gitea-bootstrap.json`, and `npm run dev:gitea-bootstrap-config`. The npm task-gateway and review-surface webhook entrypoints now resolve startup settings from that template/local bootstrap config; explicit host/port/route CLI flags remain available for manual override. The local Gitea runner, worker runtime, and CI-to-host traceability callback still have operator-controlled settings that are env-only or embedded as defaults without a dedicated template surface.
- Next Action: Decide whether runner and worker-runtime settings belong under the dev bootstrap config or dedicated runtime templates, then remove or parameterize the embedded CI host callback URL if it remains operator-controlled.
