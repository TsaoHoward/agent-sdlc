...
- Next Action: Keep this issue focused on narrower follow-ups such as a true default-bootstrap happy-path validation from issue comment through review callback and improving operator-facing artifact browsing, and only reopen forge-trigger investigation if the restored branch-source flow exposes a new isolated defect.
