...
|---|---|---|---|---|---|
| I-006 | Large-File Documentation Truncation Risk | Ready For Review | `docs/roadmap.md` Phase 1; WBS `3.9` | Review the landed safe fragment-edit support plus fail-closed guardrail for truncated-context large files, then run one live issue-comment rerun before deciding whether the WBS `3.9` blocker is cleared | Move out after the execution contract is hardened and the regression is covered by durable testing |
| I-001 | Phase 1 Minimum Closed Loop Implementation Packaging | In Progress | `docs/roadmap.md` Phase 1; WBS `3`, `3.1`-`3.11` | Apply the current close interpretation from `docs/phase1-close-checklist.md`, keep deterministic no-op coverage and bounded-code hardening deferred, but hold final WBS `3.9` close-out until `I-006` resolves the large-file documentation truncation regression | Archive after the first slice is underway and the remaining work is split or absorbed into active execution tracking |
| I-005 | Issue-Comment Failure And No-Op UX Hardening | Deferred | `docs/roadmap.md` Phase 1; WBS `3.1`, `3.8`, `3.9` | Keep the newly landed bounded issue-thread feedback path minimal in P1, then design richer user-facing status, retry guidance, duplicate-comment control, operational-state storage evolution, unified UX-discovery coverage, and a concrete DFD output mechanism as a follow-up instead of hiding it in chat | Move out after the follow-up is either promoted into active implementation or intentionally deferred beyond the current P1 packaging window |
...
- Current UX Hardening (2026-04-29): the live issue-comment path now posts bounded `agent-admin` feedback comments for fail-closed intake and failed live-continuation states, and a completed zero-edit agent result now stops before PR creation as a visible no-op instead of silently creating a traceability-only PR.
- Next Action: Use `docs/phase1-close-checklist.md` as the durable close-out path, keep the recorded judgment that deterministic no-op coverage and bounded-code hardening are deferred follow-up rather than baseline blockers, but first resolve the newly discovered large-file documentation truncation regression under `I-006` before treating WBS `3.9` as closeable.
...
- Notes: This item remains the packaging and close-out umbrella for the current Phase 1 slice; `I-006` is the newly promoted execution-quality blocker inside that umbrella.
...
- Why It Matters: A supported `documentation_update` task should not be able to pass validation while deleting unseen tail content from a large markdown file.
- Current State: Fresh evidence from issue `#41` / comment `#193` -> task `trq-097cad6b8f77` -> session `ags-2736c300be71` -> PR `#42` -> commit `83acd236a04c1d59dbf109c964e389308b53a053` showed the failure mode on `README.md`: the repo-owned execution contract truncated context files at `maxFileBytes: 8000` while still asking the provider to return complete file contents, and the current `README.md` is `11606` bytes. A narrow fail-closed guardrail has now landed in `scripts/lib/agent-execution.js`: if the provider tries to rewrite a file whose supplied context was truncated, execution now stops with an explicit error before writing the file. The same fix also adds safe non-replace edit modes (`append`, `prepend`, `insert_after`, `insert_before`) so large-file docs updates are no longer limited to silent failure or total blockage. Deterministic local stubbed verification on 2026-04-30 confirmed both behaviors: destructive replace was blocked, and `insert_after` successfully updated the truncated `README.md` while preserving the unseen tail.
- Next Action: Review the landed safe fragment-edit support plus fail-closed guardrail, then run one live issue-comment rerun to confirm the same safe behavior in the normal proposal-facing workflow before clearing the blocker.
- Exit Path: Move out after large-file docs updates either preserve untouched content safely or fail closed before proposal creation, and the behavior is covered by a durable regression case.
- Supporting Notes: `docs/issues/items/I-006-large-file-documentation-truncation-risk.md`
- Promotion / Escalation Check: Promote only if the mitigation changes architecture boundaries, source-of-truth ownership, or the execution contract in a way that deserves separate decision tracking.
- Notes: This is a quality and guardrail issue inside the current WBS `3.9` slice, not a request to broaden Phase 1 scope.
...
- Status: `Deferred`
- Related Docs / WBS: `docs/issues/items/I-005-issue-comment-failure-and-noop-ux-hardening.md`; `docs/user-guide.zh-TW.md`; `docs/testing/local-test-procedures.md`; WBS `3.1`, `3.8`, `3.9`
- Why It Matters: A live issue-comment path that only succeeds on the happy path but stays silent on malformed input, stale-forge preflight, or provider no-op results creates confusing UX and makes routine operator mistakes look like infrastructure failure.
- Current State: The minimal P1 fix is now in place: fail-closed intake and failed live-continuation states can write bounded `agent-admin` comments back into the issue thread, and a completed zero-edit agent result now stops before PR creation instead of silently creating a traceability-only PR. The richer design space is intentionally left open so P1 does not expand into a full conversational control plane by accident. The current close interpretation also treats the remaining deterministic no-op repro gap as deferred coverage work rather than a Phase 1 baseline blocker.
- Next Action: Keep the landed feedback path narrow during P1, then choose whether the next follow-up should add structured status taxonomy, duplicate-comment suppression, explicit retry links, richer no-op explanation, operational-state storage evolution beyond file-backed repo state, a more systematic UX discovery/acceptance mechanism, or a concrete DFD output mechanism for diagnosing lifecycle and UX failures.
- Exit Path: Move out after the richer UX direction is either promoted into active implementation work or explicitly deferred beyond the current P1 packaging window.
- Supporting Notes: `docs/issues/items/I-005-issue-comment-failure-and-noop-ux-hardening.md`
- Promotion / Escalation Check: Promote only if the follow-up changes workflow ownership, introduces a durable new user-control surface, or moves status/reporting authority across architecture boundaries.
- Notes: This item is intentionally about UX hardening and operator clarity, not about redefining the core layered architecture.
